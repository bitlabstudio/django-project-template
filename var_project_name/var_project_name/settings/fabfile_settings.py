...
LOGIN_USER_PROD = 'django'
HOST_DEV = None
HOST_STAGE = None
HOST_PROD = 'ENTER_IP_HERE'
...
def get_fab_setting(setting_name):
    ...
    if setting_name == 'SERVER_REPO_PROJECT_ROOT':
        # must have trailing space because of rsync
        return '{0}{1}/'.format(
            get_fab_setting('SERVER_REPO_ROOT'), PROJECT_NAME)

    if setting_name == 'SERVER_APP_ROOT':
        # must have no trailing space because of rsync
        return '/home/{0}/project'.format(
    ...
    if setting_name == 'SERVER_DB_BACKUP_DIR':
        return '/home/{0}/backups/postgres/'.format(env.user)

    if setting_name == 'SERVER_MEDIA_BACKUP_DIR':
        return '/home/{0}/backups/media/'.format(env.user)
